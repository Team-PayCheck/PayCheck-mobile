...
이 스킬은 **사용자 직접 진입 전용 wrapper**다. 사용자가 자연어로 "리뷰해줘", "검토해줘"라고 요청할 때만 트리거.

`paycheck-issue` 등 다른 스킬 흐름 내부에서는 이 스킬을 거치지 말고 `Agent(subagent_type: "paycheck-qa")`를 직접 호출한다 (한 단계 적은 호출, 동일 결과).
